...
// TODO: Create variables 
	...
	// TODO: Dependent varaibles
*** Tobin´s Q ***
. generate tq1= ((TtlCmnSharesOut * PriceClose)+ TotalDebt)/TotalAssetsReported if TotalAssetsReported>0 & TotalEquity>0 & TtlCmnSharesOut>0
. winsor tq1, generate(wtq1) p(0.015) highonly
. label variable tq1 "((#Shares Outstanding*Price)+Total Equity)/Total Assets"

. generate lntq1=ln(wtq1)
. label variable lntq1 "Ln(wtq1)"

. generate tq2= (TRCompanyMarketCap + TotalDebt)/TotalAssetsReported if TotalAssetsReported>0 & TotalEquity>0 & TtlCmnSharesOut>0
. winsor tq2, generate(wtq2) p(0.01) highonly
. label variable tq2 "(TRCompanyMarketCap+Total Equity)/Total Assets"
...
*** Industry adjusted Tobin´s Q ***
	...
	*** profitability ***
. generate roa= NetIncomeAfterTaxes/TotalAssetsReported if TotalAssetsReported>0 & TotalEquity>0
. winsor roa, generate(wroa) p(0.01)
. label variable roa "Net Income/Total Assets"

. generate roe= NetIncomeAfterTaxes/ TotalEquity if TotalAssetsReported>0 & TotalEquity>0
. winsor roe, generate(wroe) p(0.01)
. label variable roe "NI/Total Equity"

. generate pm= TotalRevenue/NetIncomeAfterTaxes if TotalRevenue>0 & TotalAssetsReported>0 & TotalEquity>0
...
*** firm size size
	. generate size=ln(TotalAssetsReported) if TotalAssetsReported>0 & TotalEquity>0
	. label variable size "Ln(Total Assets)"

*** tangibility of assets
. generate tangible= PropertyPlantEquipmentTotalNet/TotalAssetsReported if PropertyPlantEquipmentTotalNet<TotalAssetsReported & TotalAssetsReported>0 & TotalEquity>0 & PropertyPlantEquipmentTotalNet>0
. label variable tangible "Net FA / Total Assets"

*** dividends
. generate div= (DpsCommonStock* TtlCmnSharesOut)/TotalAssetsReported if TotalAssetsReported>0 & TotalEquity>0 & TtlCmnSharesOut>0
. replace div=. if div>1
. label variable div "(DPS*#Shares Outstanding)/Total Assets"

*** non-debt tax shield ***
. generate ndts= DepreciationAmort/TotalAssetsReported if TotalAssetsReported>0 & TotalEquity>0 & DepreciationAmort>=0
. replace ndts=. if ndts>0.1
. label variable ndts "Dep & Amortiz/Total Assets"



*** capital expenditure ***
. generate capex= -CapitalExpenditures/TotalAssetsReported if TotalAssetsReported>0 & TotalEquity>0
