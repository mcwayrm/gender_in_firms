...
// Ryan, please, check the construction of this variable: Industry adjusted Tobin´s Q. It is the firm’s Tobin´s Q divided by the median of the firm’s Tobin´s Q in the firm’s industry
sort id_industry_code
by id_industry_code: egen tqmedian=median(wtq1)
label variable tqmedian "Median of wtq1 by industry"
generate wtq1ind = wtq1/ tqmedian
label variable wtq1ind "wtq1 / median wtq1 by industry"

generate lntq1ind = ln(wtq1ind)
label variable lntq1ind "Ln(wtqind)"

// Return on Assest
generate roa= is_income_after_tax/ass_tot_asset if ass_tot_asset>0 & liab_tot_common_equity>0
winsor roa, generate(wroa) p(0.01)
label variable roa "Net Income/Total Assets"


// Return on Equity
generate roe= is_income_after_tax/ liab_tot_common_equity if ass_tot_asset>0 & liab_tot_common_equity>0
winsor roe, generate(wroe) p(0.01)
label variable roe "NI/Total Equity"

// Profit Margin
generate pm= is_income_after_tax/is_tot_revenue if is_tot_revenue>0 & ass_tot_asset>0 & liab_tot_common_equity>0
winsor pm, generate(wpm) p(0.02)
label variable pm "Sales/NI"



	// TODO: Create the explantaory variables: Gender diversity
// Gender Diversity at Executive Level
generate gdexec1 = cg_exec_per_female/100
label variable gdexec1 "% women on executive positions"

generate gdexec2 = 1- ((gdexec1^2)+((1-gdexec1)^2))
label variable gdexec2 "Executive Blau Index"

generate gdexec3 = abs((ln(gdexec1^gdexec1))+ln((1-gdexec1)^(1-gdexec1)))
label variable gdexec3 "Executive Shannon Index"


// Gender Diversity at Board of Directors Level
generate gdboard1 = cg_board_per_female/100
label variable gdboard1 "% women on board of directors"

generate gdboard2 = 1- ((gdboard1^2)+((1-gdboard1)^2))
label variable gdboard2 "Board Blau Index"

generate gdboard3 = abs((ln(gdboard1^gdboard1))+ln((1-gdboard1)^(1-gdboard1)))
label variable gdboard3 "Board Shannon Index"

// Gender Diversity at Middle-Management Level
generate gdmanager1 = cg_manage_per_female/100
label variable gdmanager1 "% women managers"

generate gdmanager2 = 1- ((gdmanager1^2)+((1-gdmanager1)^2))
label variable gdmanager2 "Managers Blau Index"

generate gdmanager3 = abs((ln(gdmanager1^gdmanager1))+ln((1-gdmanager1)^(1-gdmanager1)))
label variable gdmanager3 "Managers Shannon Index"

// Aggregated mesaures of Gender Diversity
generate gdtotal = (1+gdexec1)*(1+gdboard1)*(1+gdboard3)-1
label variable gdtotal "Aggregated measure of 3 levels of gender diversity"
	
	
	// TODO: Create the explantaory variables: ESG variables
// ESG score
generate esg1= esg_score/100
label variable esg1 "ESG score"

generate esg2 = esg_score_env/100
label variable esg2 "Environmental score"

generate esg3 = esg_score_soc/100
label variable esg3 "Social score"

generate esg4 = esg_score_gov/100
label variable esg4 "Governance score"

generate esg5 = (1+esg2) * (1+esg3) * (1+esg4) - 1
label variable esg5 "Aggregated measure of ESG"


	// TODO: Create control variables at firm level: 
// Firm size 
generate size=ln(ass_tot_asset) if ass_tot_asset>0 & liab_tot_common_equity>0
label variable size "Ln(Total Assets)"

// Tangibility of assets
generate tangible= ass_property_plant/ass_tot_asset if ass_property_plant<ass_tot_asset & ass_tot_asset>0 & liab_tot_common_equity>0 & ass_property_plant>0
label variable tangible "Net FA / Total Assets"

// Dividend policy
generate div= (perf_dividend_per_share* own_num_common_share_outstand)/ass_tot_asset if ass_tot_asset>0 & liab_tot_common_equity>0 & own_num_common_share_outstand>0
replace div=. if div>1
label variable div "(DPS*#Shares Outstanding)/Total Assets"

// Non-debt tax shield 
generate ndts= is_depreciation_amortization/ass_tot_asset if ass_tot_asset>0 & liab_tot_common_equity>0 & is_depreciation_amortization>=0
replace ndts=. if ndts>0.2
label variable ndts "Dep & Amortiz/Total Assets"

// Capital expenditure 
generate capex= -cfs_capital_expenditure/ass_tot_asset if ass_tot_asset>0 & liab_tot_common_equity>0
winsor capex, generate(wcapex) p(0.005)
label variable capex "CAPEX / Total Assets"


	// TODO: Create control variables at country level: 
// Deposit Money Bank Assets
generate depositgdp= cfd_country_dbagdp/100
label variable depositgdp "Deposits / GDP"

// Private Credit
generate privatecredgdp= cfd_country_pcrdbgdp/100
label variable privatecredgdp "Private Credit by Deposit Money Bank / GDP"

// Bank Deposits
generate bankdepositgdp= cfd_country_bdgdp/100
label variable bankdepositgdp "Bank Deposits / GDP"

// Bank Concentration 
generate bankconc= cfd_country_concentration/100
label variable bankconc "Bank Concentration"

// Stock Market Capitalization 
generate stockmktcap= cfd_country_stmktcap/100
label variable stockmktcap "Stock Market Capitalization / GDP"

// Stock Market Total Value Traded 
generate stockmkttvt = cfd_country_stvaltraded/100
label variable stockmkttvt "Stock Market Total Value Traded / GDP"

// Stock Market Turn-Over Ratio 
generate stockmkttor= cfd_country_stturnover/100
label variable stockmkttor "Stock Market Turn-Over Ratio / GDP"

// International Debt Issues
generate intdebtissues = cfd_country_intldebt / 100
label variable intdebtissues "International Debt Issue / GDP"
