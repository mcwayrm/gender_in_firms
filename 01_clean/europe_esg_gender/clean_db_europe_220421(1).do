...
// TODO: Create variables 
	...
	// TODO: Dependent varaibles: Firm´s financial performance
//	Tobin´s Q
generate tq1= ((own_market_cap)+ liab_tot_debt)/ass_tot_asset if ass_tot_asset>0 & liab_tot_common_equity>0
winsor tq1, generate(wtq1) p(0.01) highonly
label variable tq1 "((Market Capitalization)+Total Debt)/Total Assets"

generate lntq1=ln(wtq1)
label variable lntq1 "Ln(wtq1)"

// Market to book ratio
generate tq2= own_market_cap/liab_tot_common_equity if ass_tot_asset>0 & liab_tot_common_equity>0 & own_market_cap>0
winsor tq2, generate(wtq2) p(0.01) 
label variable tq2 "Total Assets / Totque Equity"

generate lntq2=ln(wtq2)
label variable lntq2 "Ln(wtq2)"
...
*** COMPUTATION OF ADDITIVE INDEXES FOR ESG STRATEGY ***
. alpha esg_score_env esg_score_soc esg_score_gov, std
. alpha esg_score_env_resource esg_score_env_emit esg_score_env_innovate esg_score_soc_workforce esg_score_soc_rights esg_score_soc_community esg_score_soc_prod_responsible esg_score_gov_manage esg_score_gov_shareholder esg_score_gov_strategy esg_score_water esg_score_energy esg_score_package esg_score_supply_chain, std

. generate INDEXESG1 = (esg_score_env + esg_score_soc + esg_score_gov) / 300
. label variable INDEXESG1 "Index ESG based on 3 pillars"
. histogram INDEXESG1, normal

. generate INDEXESG2 = (esg_score_env_resource + esg_score_env_emit + esg_score_env_innovate + esg_score_soc_workforce + esg_score_soc_rights + esg_score_soc_community + esg_score_soc_prod_responsible + esg_score_gov_manage + esg_score_gov_shareholder + esg_score_gov_strategy + esg_score_water + esg_score_energy + esg_score_package + esg_score_supply_chain) / 1400
. label variable INDEXESG2 "Index ESG based on 14 scores"
. histogram INDEXESG2, normal

. generate INDEXESG1_D = 1 if INDEXESG1_N>=0
. replace INDEXESG1_D = 0 if INDEXESG1_N<0
. replace INDEXESG1_D = . if INDEXESG1_N==.
. label variable INDEXESG1_D "Dummy Index ESG based on 3 pillars"

*** NORMALIZATION OF THE ESG STRATEGY INDEXES ***
. summarize INDEXESG1
. generate INDEXESG1_N = (INDEXESG1 - r(mean)) / r(sd)
. summarize INDEXESG1 INDEXESG1_N
. label variable INDEXESG1_N "Normalized INDEXESG1 with mean 0 and sd 1"
. order INDEXESG1_N, after(INDEXESG1)

. summarize INDEXESG2
. generate INDEXESG2_N = (INDEXESG2 - r(mean)) / r(sd)
. summarize INDEXESG2 INDEXESG2_N
. label variable INDEXESG2_N "Normalized INDEXESG2 with mean 0 and sd 1"
. order INDEXESG2_N, after(INDEXESG2)

. generate INDEXESG2_D = 1 if INDEXESG2_N>=0
. replace INDEXESG2_D = 0 if INDEXESG2_N<0
. replace INDEXESG2_D = . if INDEXESG2_N==.
. label variable INDEXESG2_D "Dummy Index ESG based on 14 scores"

*** COMPUTATION OF GENDER VARIABLES ***
. generate BGD1 = cg_board_per_female/100
. label variable BGD1 "% of female on board"

. generate BGD2 = 1- ((BGD1^2)+((1-BGD1)^2))
. label variable BGD2 "Board Blau Index"

. generate BGD3 = abs((ln(BGD1^BGD1))+ln((1-BGD1)^(1-BGD1)))
. label variable BGD3 "Board Shannon Index"

. generate EGD1 = cg_exec_per_female/100
. label variable EGD1 "% of female executives"

. generate EGD2 = 1- ((EGD1^2)+((1-EGD1)^2))
. label variable EGD2 "Executive Blau Index"

. generate EGD3 = abs((ln(EGD1^EGD1))+ln((1-EGD1)^(1-EGD1)))
. label variable EGD3 "Executive Shannon Index"

. generate MGD1 = cg_manage_per_female/100
. label variable MGD1 "% of female managers"

. generate MGD2 = 1- ((MGD1^2)+((1-MGD1)^2))
. label variable MGD2 "Managers Blau Index"

. generate MGD3 = abs((ln(MGD1^MGD1))+ln((1-MGD1)^(1-MGD1)))
. label variable MGD3 "Managers Shannon Index"


*** COMPUTATION OF BOARD FEATURES ***
. generate BIndep = cg_board_perc_independent/100
. label variable BIndep "% independent board members"

. generate BExperience = cg_board_experience
. label variable BExperience "Avg. number of year on board"

. generate BNonExec = cg_board_per_nonexec/100
. label variable BNonExec "% of nonexecutive board members"

. generate BAttendance = cg_board_meeting_attendance_avg/100
. label variable BAttendance "Avrg. attendance in board meetings"

. winsor cg_board_tot_compensate, generate(BComp_w1) p(0.01)
. generate BCompDir = BComp_w1 / cg_board_size
. label variable BCompDir "Avrg. Compensation per director"

. generate BLnCompDir = ln(BCompDir)
. label variable BLnCompDir "Ln(BCompDir)"

. winsor cg_top_exec_tot_compensate, generate (EComp_w1) p(0.01)
. generate ECompRev = EComp_w1/ is_tot_revenue
. label variable ECompRev "Executive Comp. / Total Revenues"
. winsor ECompRev, generate (ECompRev_w1) p(0.01)

. generate ELnCompRev = ln(ECompRev_w1)
. label variable ELnCompRev "Ln(ECompRev_w1)"

. generate BSeparation = cg_board_chairman
. label variable BSeparation "1 if no separation betwen CEO & chairman, 0 otherwise"

. generate BCEOnoBoard = cg_board_ceo
. label variable BCEOnoBoard "1 if CEO is on the board, 0 otherwise"


*** COMPUTATION OF BOARD-CORPORATE GOVERNANCE INDEX AND ITS NORMALIZATION ***

. alpha cg_board_diversity_policy cg_board_experience cg_board_meeting_attendance cg_committee_governance cg_committee_nominate cg_committee_audit cg_committee_compensate cg_exec_compensate_tools cg_exec_compensate_vote cg_ceo_compensate_share_return cg_ceo_compensate_csr cg_exec_compensate_tools BSeparation BCEOnoBoard, std
. generate INDEXBCG = (cg_board_diversity_policy + cg_board_experience + cg_board_meeting_attendance + cg_committee_governance + cg_committee_nominate + cg_committee_audit + cg_committee_compensate + cg_exec_compensate_tools + cg_exec_compensate_vote + cg_ceo_compensate_share_return + cg_ceo_compensate_csr + cg_exec_compensate_tools + BSeparation + BCEOnoBoard)/13
. label variable INDEXBCG "Index B-CG based on 13 indicators"

. summarize INDEXBCG
. generate INDEXBCG_N = (INDEXBCG - r(mean)) / r(sd)
. summarize INDEXBCG INDEXBCG_N
. label variable INDEXBCG_N "Normalized INDEXBCG with mean 0 and sd 1"
. order INDEXBCG INDEXBCG_N, after(INDEXESG2_N)


*** COMPUTATION OTHER INDEPENDENT VARIABLES ***
. generate fv1 = (own_market_cap + liab_tot_debt) / ass_tot_asset if liab_tot_common_equity>0
. label variable fv1 "(MkCap+TD)/TA"
. winsor2 fv1, suffix(_w2) cuts(0 99) trim
. label variable fv1_w2 "fv1 trimmed at 1%"

. generate lnfv1 = ln(fv1_w2)
. label variable lnfv1 "ln(fv1_w2)"

. generate fv2 = (own_market_cap  + liab_tot_liab) / ass_tot_asset if liab_tot_common_equity>0
. label variable fv2 "(MkCap+Total Liabilities)/TA"
. winsor2 fv2, suffix(_w2) cuts(0 99) trim
. label variable fv2_w2 "fv2 trimmed at 1%"

. generate lnfv2 = ln(fv2_w2)
. label variable lnfv2 "ln(fv2_w2)"

. generate size = ln(ass_tot_asset) if liab_tot_common_equity>0
. label variable size "Ln(TA)"
. swilk size

. generate lev1 = liab_tot_debt / ass_tot_asset if liab_tot_common_equity>0
. label variable lev1 "TD / TA"
. swilk lev1

. generate lev2 = liab_tot_liab / ass_tot_asset if liab_tot_common_equity>0
. label variable lev2 "Total Liabilities / TA"
. swilk lev2
. histogram lev2, normal

. generate ROA = is_income_after_tax/ ass_tot_asset if liab_tot_common_equity>0
. label variable ROA "NI / TA"
. winsor ROA, generate(ROA_w) p(0.01)
. winsor2 ROA, suffix(_w2) cuts(1 99) trim
. label variable ROA_w2 "ROA trimmed at 1%"

. generate ROE = is_income_after_tax/liab_tot_common_equity if liab_tot_common_equity>0
. label variable ROE "NI / Equity"
. winsor ROE, generate(ROE_w) p(0.01)
. winsor2 ROE, suffix(_w2) cuts(1 99) trim
. label variable ROE_w2 "ROE trimmed at 1%"

. generate CAPEX = -cfs_capital_expenditure / ass_tot_asset if liab_tot_common_equity>0
. label variable CAPEX "Purchase of FA"
. winsor2 CAPEX, suffix(_w2) cuts(1 99) trim
. label variable CAPEX_w2 "CAPEX trimmed at 1%"


*** COMPUTATION OF COUNTRY GOVERNANCE INDEX ***
. correlate wgi_country_accountability wgi_country_political_stable wgi_country_gov_effective wgi_country_reg_quality wgi_country_rule_law wgi_country_control_corrupt
. alpha wgi_country_accountability wgi_country_political_stable wgi_country_gov_effective wgi_country_reg_quality wgi_country_rule_law wgi_country_control_corrupt, std
. generate wgi = (wgi_country_accountability + wgi_country_political_stable + wgi_country_gov_effective + wgi_country_reg_quality + wgi_country_rule_law + wgi_country_control_corrupt + 15)/30
. label variable wgi "World Governance Index (0-1)"
. order wgi, before(wgi_country_accountability)
. histogram wgi, normal

. factor wgi_country_accountability wgi_country_political_stable wgi_country_gov_effective wgi_country_reg_quality wgi_country_rule_law wgi_country_control_corrupt, pcf
. rotate
. predict factorwgi 
. summarize factorwgi
. correlate wgi_country_accountability wgi_country_political_stable wgi_country_gov_effective wgi_country_reg_quality wgi_country_rule_law wgi_country_control_corrupt factorwgi
. label variable factorwgi "Factor World Governance Index"
. order factorwgi, before(wgi_country_accountability)


*** COMPUTATION OF TEMPORAL, INDUSTRY, AND COUNTRY DUMMY VARIABLES ***
