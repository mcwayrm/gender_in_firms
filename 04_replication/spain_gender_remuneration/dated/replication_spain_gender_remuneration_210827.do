...
pantob EQ1 GrDivers1 BRemun5 BRemun52 BOwn BIndependent BSize1 BExecutive Size wZScore wroa leverage1 TQ factor1 factor2 factor3 Iden
	...
	twoway (lpolyci extrema_predict BRemun5 $cond_if, bwidth(2) degree(2) yaxis(1)) ///
		...
		xtitle("Compensation per Director (in Euros)") ytitle("Earnings Quality (EQ1)") ///
...
twoway function y = .83897*x - .03218*(x^2), ///
	range(0 20) ///
	xline(8.2017183) ///
	text(1 8.4 "Avg. Remuneration", size(small) place(e)) ///
	ytitle("Earning Quality (EQ1)") xtitle("Board Remuneration (BREM1)") ///
	name("plot_extrema", replace)
