...
// Create Coefficent Plot for GD Robustness 
	...
	coefplot (plot_reg_eq1_gd1 plot_reg_eq1_gd2 plot_reg_eq1_gd3, ciopts(lcolor(black))) ///
		(plot_reg_eq2_gd1 plot_reg_eq2_gd2 plot_reg_eq2_gd3, ciopts(lcolor(blue))) ///
		(plot_reg_eq3_gd1 plot_reg_eq3_gd2 plot_reg_eq3_gd3, ciopts(lcolor(gold))) ///
		(plot_reg_eq4_gd1 plot_reg_eq4_gd2 plot_reg_eq4_gd3, ciopts(lcolor(green))) ///
		, $opts ///
		legend(order(1 "EQ1" 3 "EQ2" 5 "EQ3" 7 "EQ4") title("Earning Quality Measure") cols(4) pos(6)) ///
		title("Figure 1: Gender Diversity Influence on Earnings Quality", pos(12)) ///
...
// TODO: 1 graph to show the distribution of the EQ measures for the extrema point (show the ridge that maximizes for the optimization), 4 lines representing the difference of REM 1 - 4, Thicken line for REM1, dashed line with average board remuneration (try to show some above and below, either with a box plot or lines for quartiles)
	...
	// TODO: Equation specification, yaxis is 0-1, xaxis in Euros, add 95% confidence intervals (replace coefficents from those in reg CI estimates)

// Main Regression 
pantob EQ1 GrDivers1 BRemun5 BRemun52 BOwn BIndependent BSize1 BExecutive Size wZScore wroa leverage1 TQ factor1 factor2 factor3 Iden
	
	// Predict EQ Value
	predict extrema_predict, xb
	...
	// Alternative Version of showing Optimization
	twoway (lpolyci extrema_predict BRemun5 $cond_if, bwidth(2) degree(2) yaxis(1)) ///
		(hist BRemun5 $cond_if, width(.5) percent yaxis(2)), ///
		legend(order(2 "Smoothed Linear Prediction" 1 "95% CI"  3 "Frequency of Firms") cols(3) pos(6)) /// 
		xtitle("Log Scale of Compensation per Director (in Euros)") ytitle("Earnings Quality (EQ1)") ///
		ylabel(2.2 "0" 2.4 ".2" 2.6 ".4" 2.8 ".6" 3 ".8" 3.2 "1") ///
		xlabel(7 "1,096" 9 "8,103" 11 "59,874" 13 "442,413" 15 "3,269,017") ///
		name("plot_extrema", replace)
	
	
	// Add the CI for the function
	// Add the Histogram for this
	// line/dot for value of optimization
	// darker color for bars above optimal point
	// not in graph: % firms above the optimization point
	
	// Y-Axis: remove the numbers (hide ticks)
	// X-axis: leave in log, dont convert to euros
	
// Graphically show Optimization of EQ1 based on BRemun5 
twoway (function y = .83897*x - .03218*(x^2), range(0 20) yaxis(1))
 ///
...
// 	xline(8.2017183) ///
// 	text(1 8.4 "Avg. Remuneration", size(small) place(e)) ///
